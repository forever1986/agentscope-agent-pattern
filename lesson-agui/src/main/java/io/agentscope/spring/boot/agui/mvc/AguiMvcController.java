...
public class AguiMvcController {
    ...
    private SseEmitter handleInternal(
            RunAgentInput input, String headerAgentId, String pathAgentId) {
        SseEmitter emitter = sseEmitterMap.computeIfAbsent(input.getRunId(), k -> new SseEmitter(sseTimeout));
                        ...
                        subscription =
                                result.events()
                                        .subscribe(
                                                ...
                                                () -> {
                                                    try {
                                                        AguiEvent aguiEvent = new AguiEvent.TextMessageEnd(threadId,runId,"test");
